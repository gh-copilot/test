...
class KNNIdentification:
    ...
    def __init__(self, n_classes: int = -1, k=5, threshold=12.5, conflict_solving_strategy=None):
        """
    ...
    def knn(self, faces: np.ndarray, faces_positions=None) -> npt.NDArray[np.uint16]:
        ...
        if self.conflict_solving_strategy is not None and np.unique(classes).shape[0] != num_of_test_faces:
    ...
    def get_ids(self, faces: List[np.ndarray], faces_positions=None) -> npt.NDArray[np.uint16]:
        ...
        eigen_faces = FeatureExtraction.eigen_faces_features(faces)
